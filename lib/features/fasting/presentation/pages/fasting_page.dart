...
class _FastingPageState extends State<FastingPage> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<FastingBloc, FastingState>(
      listener: (context, state) {
        if (state.errorMessage == 'É obrigado a parar o jejum atual') {
          showAppToast(context, state.errorMessage, type: AppToastType.info);
        }
      },
              ...
              Wrap(
                ...
                children: [
                  ...
                  ChoiceChip(
                    ...
                    onSelected: (_) async {
                      if (_isFastingActive(state)) {
                        showAppToast(
                          context,
                          'É obrigado a parar o jejum atual',
  ...
  Widget _protocolChip(
    ...
    final fastingActive = _isFastingActive(state);
    return ChoiceChip(
      label: Text(protocol.label),
      selected: selected,
      onSelected: (_) {
        if (fastingActive && !selected) {
          showAppToast(
            context,
            'É obrigado a parar o jejum atual',
            type: AppToastType.info,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _timerRow('Status', switch (state.session.status) {
  ...
  bool _isFastingActive(FastingState state) {
    return state.session.status == FastingSessionStatus.running ||
  ...
  Future<FastingProtocol?> _showCustomProtocolDialog(
    BuildContext context,
  ) async {
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Horas de alimentação',
                ),
              ),
            ],
          ),
